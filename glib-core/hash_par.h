...
const unsigned int THashPar<TKey, TDat, THashFunc>::HashPrimeT[HashPrimes]={
  3ul, 5ul, 11ul, 23ul,
...
template<class TKey, class TDat, class THashFunc>
uint THashPar<TKey, TDat, THashFunc>::GetNextPrime(const uint& Val) const {
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::Resize(){
...
template<class TKey, class TDat, class THashFunc>
THashPar<TKey, TDat, THashFunc>::THashPar(const int& ExpectVals, const bool& _AutoSizeP):
...
template<class TKey, class TDat, class THashFunc>
bool THashPar<TKey, TDat, THashFunc>::operator==(const THashPar& Hash) const {
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::Clr(const bool& DoDel, const int& NoDelLim, const bool& ResetDat){
  if (DoDel){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::DelKey(const TKey& Key){
  IAssert(!PortV.Empty());
  const int PortN=abs(THashFunc::GetPrimHashCd(Key)%PortV.Len());
  const int HashCd=abs(THashFunc::GetSecHashCd(Key));
  int PrevKeyId=-1;
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::MarkDelKey(const TKey& Key){
...
template<class TKey, class TDat, class THashFunc>
int THashPar<TKey, TDat, THashFunc>::GetRndKeyId(TRnd& Rnd) const  {
  IAssert(! Empty());
...
template<class TKey, class TDat, class THashFunc>
int THashPar<TKey, TDat, THashFunc>::GetRndKeyId(TRnd& Rnd, const double& EmptyFrac) {
  IAssert(! Empty());
...
template<class TKey, class TDat, class THashFunc>
int THashPar<TKey, TDat, THashFunc>::GetKeyId(const TKey& Key) const {
...
template<class TKey, class TDat, class THashFunc>
bool THashPar<TKey, TDat, THashFunc>::FNextKeyId(int& KeyId) const {
  do {KeyId++;} while ((KeyId<KeyDatV.Len())&&(KeyDatV[KeyId].HashCd==-1));
  return KeyId<KeyDatV.Len();
}

template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetKeyV(TVec<TKey>& KeyV) const {
  KeyV.Gen(Len(), 0);
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetDatV(TVec<TDat>& DatV) const {
  DatV.Gen(Len(), 0);
  int KeyId=FFirstKeyId();
  while (FNextKeyId(KeyId)){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetKeyDatPrV(TVec<TPair<TKey, TDat> >& KeyDatPrV) const {
  KeyDatPrV.Gen(Len(), 0);
  TKey Key; TDat Dat;
  int KeyId=FFirstKeyId();
  while (FNextKeyId(KeyId)){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetDatKeyPrV(TVec<TPair<TDat, TKey> >& DatKeyPrV) const {
  DatKeyPrV.Gen(Len(), 0);
  TKey Key; TDat Dat;
  int KeyId=FFirstKeyId();
  while (FNextKeyId(KeyId)){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetKeyDatKdV(TVec<TKeyDat<TKey, TDat> >& KeyDatKdV) const {
  KeyDatKdV.Gen(Len(), 0);
  TKey Key; TDat Dat;
  int KeyId=FFirstKeyId();
  while (FNextKeyId(KeyId)){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::GetDatKeyKdV(TVec<TKeyDat<TDat, TKey> >& DatKeyKdV) const {
  DatKeyKdV.Gen(Len(), 0);
  TKey Key; TDat Dat;
  int KeyId=FFirstKeyId();
  while (FNextKeyId(KeyId)){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::Swap(THashPar& Hash) {
  if (this!=&Hash){
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::Defrag(){
  if (!IsKeyIdEqKeyN()){
    THashPar<TKey, TDat, THashFunc> Hash(PortV.Len());
...
template<class TKey, class TDat, class THashFunc>
void THashPar<TKey, TDat, THashFunc>::Sort(const bool& CmpKey, const bool& Asc) {
